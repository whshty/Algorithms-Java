class Solution {
    public boolean pyramidTransition(String bottom, List<String> list) {
        Map<String,List<String>> map = new HashMap<>();
        for ( String str : list ){
            String key = str.substring(0,2);
            if(!map.containsKey(key)) map.put(key,new ArrayList<>());
            map.get(key).add(str.substring(2));
        }
        return helper(bottom,map);
    }

    private boolean helper(String bottom, Map<String,List<String>> map){
        if( bottom.length() == 1 ) return true;
        for( int i = 0 ; i < bottom.length() - 1; i++ ){
            if( !map.containsKey(bottom.substring(i,i+2))) return false;
        }
        List<String> list = new ArrayList<>();
        getList(bottom, 0, new StringBuilder(), list, map);
        for ( String str : list ){
            if( helper(str,map)) return true;
        }
        return false;
    }
    private void getList(String bottom, int index, StringBuilder sb, List<String> list, Map<String, List<String>> map) {
        if (index == bottom.length() - 1) {
            list.add(sb.toString());
            return;
        }
        for (String str : map.get(bottom.substring(index, index+2))) {
            sb.append(str);
            getList(bottom, index + 1, sb, list, map);
            sb.deleteCharAt(sb.length()-1);
        }
    }
}
